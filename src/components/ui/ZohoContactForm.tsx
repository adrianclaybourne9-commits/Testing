...
export default function ZohoContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Since validation passed, submit natively to the hidden iframe
    if (formRef.current) {
      formRef.current.submit();
    }

    // Show a realistic loading state before success
    setTimeout(() => {
          ...
          <form id="webform549617000000572187" name="WebToLeads549617000000572187" action="https://crm.zoho.in/crm/WebToLeadForm" method="POST" target="zoho_hidden_iframe" ref={formRef} onSubmit={handleSubmit} className="space-y-6" acceptCharset="UTF-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  maxLength={40}
                  required
                  className="w-full bg-black/40 border border-white/10 rounded-xl px-5 py-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all"
                  placeholder="John"
                />
              </div>
            ...
            <div className="space-y-2">
              ...
              <select
                id="LEADCF5"
                name="LEADCF5"
                className="w-full bg-black/40 border border-white/10 rounded-xl px-5 py-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all appearance-none"
              >
                <option value="-None-" className="bg-gray-900 text-gray-400">- Select a Product -</option>
                <option value="WHMCS Integrator" className="bg-gray-900 text-white">WHMCS Integrator</option>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                name="Description"
                rows={5}
                className="w-full bg-black/40 border border-white/10 rounded-xl px-5 py-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all resize-none"
